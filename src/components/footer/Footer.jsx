import React from 'react';
import { useNavigate } from 'react-router-dom';
import Levand from '../../assets/maint.png';
import { useTranslation } from 'react-i18next';
import { useContext } from 'react';
import { AppContextProv } from '../../context/AppContext';

const Footer = () => {
    const navigate = useNavigate();
    const { products } = useContext(AppContextProv)
    const { t } = useTranslation()
    return (
        <div>
            <footer className="footer p-10 bg-base-200 text-base-content">
                <div>
                    <div className='p-5'>
                        <img src={Levand} alt="LevandGıda" width={100} onClick={() => navigate("/")} className='cursor-pointer' />
                        <p className="font-bold">
                            Levand Gıda <br />{t('provides_reliable_food')}
                        </p>
                    </div>
                </div>
                <div>
                    <span className="footer-title">{t('services')}</span>
                    {
                        products?.map((item, index) => (
                            <span className="link link-hover" onClick={() => navigate('/products', { state: { products: item }, replace: false })} key={index}>{item.productCategory}</span>

                        ))
                    }
                    {/* <a className="link link-hover">{t('branding')}</a>
                    <a className="link link-hover">{t('design')}</a>
                    <a className="link link-hover">{t('marketing')}</a>
                    <a className="link link-hover">{t('advertisement')}</a> */}
                </div>
                <div>
                    <span className="footer-title">{t('company')}</span>
                    <p className="link link-hover" onClick={() => navigate("/about")}>{t('about_us')}</p>
                    <p className="link link-hover" onClick={() => navigate("/contact")}>{t('contact_us')}</p>

                </div>
                {/* <div> */}
                {/* <span className="footer-title">Our Social Accounts</span> */}
                {/* <span className="footer-title"></span> */}
                {/* <a className="link link-hover"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" className="fill-current"><path d="M24 4.557c-.883.392-1.832.656-2.828.775 1.017-.609 1.798-1.574 2.165-2.724-.951.564-2.005.974-3.127 1.195-.897-.957-2.178-1.555-3.594-1.555-3.179 0-5.515 2.966-4.797 6.045-4.091-.205-7.719-2.165-10.148-5.144-1.29 2.213-.669 5.108 1.523 6.574-.806-.026-1.566-.247-2.229-.616-.054 2.281 1.581 4.415 3.949 4.89-.693.188-1.452.232-2.224.084.626 1.956 2.444 3.379 4.6 3.419-2.07 1.623-4.678 2.348-7.29 2.04 2.179 1.397 4.768 2.212 7.548 2.212 9.142 0 14.307-7.721 13.995-14.646.962-.695 1.797-1.562 2.457-2.549z"></path></svg></a> */}
                {/* <a className="link link-hover"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" className="fill-current"><path d="M9 8h-3v4h3v12h5v-12h3.642l.358-4h-4v-1.667c0-.955.192-1.333 1.115-1.333h2.885v-5h-3.808c-3.596 0-5.192 1.583-5.192 4.615v3.385z"></path></svg></a> */}
                {/* <a className="link link-hover"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" className="fill-current"><path d="M19.615 3.184c-3.604-.246-11.631-.245-15.23 0-3.897.266-4.356 2.62-4.385 8.816.029 6.185.484 8.549 4.385 8.816 3.6.245 11.626.246 15.23 0 3.897-.266 4.356-2.62 4.385-8.816-.029-6.185-.484-8.549-4.385-8.816zm-10.615 12.816v-8l8 3.993-8 4.007z"></path></svg></a> */}
                {/* </div> */}
            </footer>

            <footer className="footer footer-center p-10 bg-lime-700 text-primary-content">
                <div>
                    <img src={Levand} alt="LevandGıda" width={80} />
                    <p className="font-bold">
                        Levand Gıda <br />{t('provides_reliable_food')}
                    </p>
                </div>
                <div>
                    <div className="grid grid-flow-col gap-4">
                        <span><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" className="fill-current"><path d="M24 4.557c-.883.392-1.832.656-2.828.775 1.017-.609 1.798-1.574 2.165-2.724-.951.564-2.005.974-3.127 1.195-.897-.957-2.178-1.555-3.594-1.555-3.179 0-5.515 2.966-4.797 6.045-4.091-.205-7.719-2.165-10.148-5.144-1.29 2.213-.669 5.108 1.523 6.574-.806-.026-1.566-.247-2.229-.616-.054 2.281 1.581 4.415 3.949 4.89-.693.188-1.452.232-2.224.084.626 1.956 2.444 3.379 4.6 3.419-2.07 1.623-4.678 2.348-7.29 2.04 2.179 1.397 4.768 2.212 7.548 2.212 9.142 0 14.307-7.721 13.995-14.646.962-.695 1.797-1.562 2.457-2.549z"></path></svg></span>
                        <span><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" className="fill-current"><path d="M19.615 3.184c-3.604-.246-11.631-.245-15.23 0-3.897.266-4.356 2.62-4.385 8.816.029 6.185.484 8.549 4.385 8.816 3.6.245 11.626.246 15.23 0 3.897-.266 4.356-2.62 4.385-8.816-.029-6.185-.484-8.549-4.385-8.816zm-10.615 12.816v-8l8 3.993-8 4.007z"></path></svg></span>
                        <span><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" className="fill-current"><path d="M9 8h-3v4h3v12h5v-12h3.642l.358-4h-4v-1.667c0-.955.192-1.333 1.115-1.333h2.885v-5h-3.808c-3.596 0-5.192 1.583-5.192 4.615v3.385z"></path></svg></span>
                    </div>
                </div>
            </footer>

            <footer className="footer items-center justify-center px-5 py-1 border-t bg-base-200 text-base-content border-base-300">
                <div className="items-center grid-flow-col">
                    <p className='text-xs'>Reliable Tech - Fatih GÜNAYDIN - Copyright © 2022 - All right reserved</p>
                    <a title="Fatih's LinkedIn" href="https://www.linkedin.com/in/fatih-gunaydin/" target={'_blank'} rel="noreferrer" ><svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24" className="fill-current text-blue-700"><g> <path fill="none" d="M0 0h24v24H0z" /> <path fillRule="nonzero" d="M6.94 5a2 2 0 1 1-4-.002 2 2 0 0 1 4 .002zM7 8.48H3V21h4V8.48zm6.32 0H9.34V21h3.94v-6.57c0-3.66 4.77-4 4.77 0V21H22v-7.93c0-6.17-7.06-5.94-8.72-2.91l.04-1.68z" /> </g></svg></a>
                    <a title="Fatih's GitHub" href="https://github.com/FatihG34" target={'_blank'} rel="noreferrer"><svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 512 512" className="fill-current"><path d="M256,32C132.3,32,32,134.9,32,261.7c0,101.5,64.2,187.5,153.2,217.9a17.56,17.56,0,0,0,3.8.4c8.3,0,11.5-6.1,11.5-11.4,0-5.5-.2-19.9-.3-39.1a102.4,102.4,0,0,1-22.6,2.7c-43.1,0-52.9-33.5-52.9-33.5-10.2-26.5-24.9-33.6-24.9-33.6-19.5-13.7-.1-14.1,1.4-14.1h.1c22.5,2,34.3,23.8,34.3,23.8,11.2,19.6,26.2,25.1,39.6,25.1a63,63,0,0,0,25.6-6c2-14.8,7.8-24.9,14.2-30.7-49.7-5.8-102-25.5-102-113.5,0-25.1,8.7-45.6,23-61.6-2.3-5.8-10-29.2,2.2-60.8a18.64,18.64,0,0,1,5-.5c8.1,0,26.4,3.1,56.6,24.1a208.21,208.21,0,0,1,112.2,0c30.2-21,48.5-24.1,56.6-24.1a18.64,18.64,0,0,1,5,.5c12.2,31.6,4.5,55,2.2,60.8,14.3,16.1,23,36.6,23,61.6,0,88.2-52.4,107.6-102.3,113.3,8,7.1,15.2,21.1,15.2,42.5,0,30.7-.3,55.5-.3,63,0,5.4,3.1,11.5,11.4,11.5a19.35,19.35,0,0,0,4-.4C415.9,449.2,480,363.1,480,261.7,480,134.9,379.7,32,256,32Z"></path></svg></a>
                </div>
                {/* <div className="md:place-self-center md:justify-self-end">
                    <div className="grid grid-flow-col gap-3">
                        <a title="Fatih's YouTube" href="https://www.youtube.com/@fatihgunaydin" target={'_blank'}><svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24" className="fill-current"><path d="M19.615 3.184c-3.604-.246-11.631-.245-15.23 0-3.897.266-4.356 2.62-4.385 8.816.029 6.185.484 8.549 4.385 8.816 3.6.245 11.626.246 15.23 0 3.897-.266 4.356-2.62 4.385-8.816-.029-6.185-.484-8.549-4.385-8.816zm-10.615 12.816v-8l8 3.993-8 4.007z"></path></svg></a>
                        <a title="Fatih's Twitter" href='https://twitter.com/Fatih_Gnydn' target={'_blank'}><svg xmlns="http://www.w3.org/2000/svg" width="12" height="15" viewBox="0 0 24 24" className="fill-current"><path d="M24 4.557c-.883.392-1.832.656-2.828.775 1.017-.609 1.798-1.574 2.165-2.724-.951.564-2.005.974-3.127 1.195-.897-.957-2.178-1.555-3.594-1.555-3.179 0-5.515 2.966-4.797 6.045-4.091-.205-7.719-2.165-10.148-5.144-1.29 2.213-.669 5.108 1.523 6.574-.806-.026-1.566-.247-2.229-.616-.054 2.281 1.581 4.415 3.949 4.89-.693.188-1.452.232-2.224.084.626 1.956 2.444 3.379 4.6 3.419-2.07 1.623-4.678 2.348-7.29 2.04 2.179 1.397 4.768 2.212 7.548 2.212 9.142 0 14.307-7.721 13.995-14.646.962-.695 1.797-1.562 2.457-2.549z"></path></svg></a>
                    </div>
                </div> */}
            </footer>
        </div>
    )
}

export default Footer
